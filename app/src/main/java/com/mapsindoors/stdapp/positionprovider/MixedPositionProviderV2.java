 ...
 */
public class MixedPositionProviderV2 implements PositionProvider
{


	public static final String TAG = MixedPositionProviderV2.class.getSimpleName();
	 ...
	 */
	public MixedPositionProviderV2(@NonNull Context context, @Nullable List<PositionProvider> positionProviders )
	{
		mContext = context;
			...
			for( InternalPosProvider ipp : mInternalPositionProviders) {
				if( ipp != null ) {
					if( !ipp.positionProvider.isRunning() ) {
						//posProvider.stopPositioning( null );
						ipp.positionProvider.startPositioning( arg );
					}
				}
			}
		}*/
...
/*
		if( mInternalPositionProviders != null ) {
			for( InternalPosProvider ipp : mInternalPositionProviders) {
				if( ipp != null ) {
					if( ipp.positionProvider.isRunning() ) {
						ipp.positionProvider.stopPositioning( arg );
					}
				}
			}
		}*/
	 ...
	 *
	 *
	 * TODO: Should we return true if ALL are running or just ANY running?
	 *
	 * @return T/F
	 */
	@Override
...
/*
		...
		if( mInternalPositionProviders.length > 0 ) {

			int runningCount = 0;

			for( InternalPosProvider ipp : mInternalPositionProviders) {
				if( ipp.positionProvider != null ) {
					if( ipp.positionProvider.isRunning() ) {
						anyRunning = true;
						runningCount++;
					}
			...
			allRunning = runningCount == mInternalPositionProviders.length;
		}*/
	...
	// TODO: 09-11-2017 add a lock to prevent an overlap
	@Override
	...
	synchronized void onPositionUpdate( @NonNull PositionResult positionResult, @NonNull PositionProvider provider, @IntRange(from = 0, to = Integer.MAX_VALUE) int ppIndex ) {
		...
		InternalPosProvider bestProvider = null;

		//InternalPosProvider mBestProvider, mPrevBestProvider
		// InternalPosProvider mBestProvider, mPrevBestProvider
//		mBestProvider = getBestProvider();
...
//		if( provider instanceof GoogleAPIPositionProvider ) {
//
//		} else if( provider instanceof BeaconPositionProvider ) {
//
//		} else if( provider instanceof CiscoPositionProvider ) {
//
//		} else {
