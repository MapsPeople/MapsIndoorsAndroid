...
public class GoogleAnalyticsManager
{
	...
	public static void reportSearch( @NonNull LocationQuery locationQuery, long count )
	{
		if( isTrackingEnabled() )
		{
			String scope;
			if( (locationQuery.getCategories() != null) && (locationQuery.getCategories().size() > 0) ) {
				scope = TextUtils.join( "_", locationQuery.getCategories().toArray() );
			} else {
				scope = FIR_GLOBAL;
			}

			final String queryString = locationQuery.getQuery();
