...
public class FloatingAction {
    ...
    public FloatingAction(final Activity activity, FloatingActionListener fabListener, AppConfigManager appConfigManager, View view) {
        ...
        mIsActive = true;
        mFABButtonHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50, activity.getResources().getDisplayMetrics());
        ...
        if (MapsIndoorsUtils.isNullOrEmpty(fabmenuEntries)) {
            ...
            for (int i = 0; i < fabEntriesCount; i++) {
                final MenuInfo mi = fabmenuEntries.get(i);
                final FabButtonWithLabel fabButtonWL = view.findViewById(mFabCategoryButtonIds[i]);
                ...
                if ((mi != null) && (fabButtonWL != null)) {
                    ...
                    final String catName = mi.getName();
                    fabButtonWL.setLabelText(catName);
